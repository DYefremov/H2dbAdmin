...
public class H2Recover implements DataHandler, Recover {
    ...
    public void cancel() {
        logger.info("H2Recover [cancel]: Canceling the task...");
        setRunning(false);
    }
    ...
    public static InputStream readBlobMap(Connection conn, long lobId, long precision) throws SQLException {
        ...
        final PreparedStatement prep = conn.prepareStatement("SELECT DATA FROM INFORMATION_SCHEMA.LOB_BLOCKS " +
    ...
    static class PageInputStream extends InputStream {
        ...
        public PageInputStream(PrintWriter writer, DataHandler handler, FileStore store, int logKey, long firstTrunkPage,
        ...
        private void fillBuffer() {
            ...
            if (dataPages.size() > 0) {
                ...
                if (t != Page.TYPE_STREAM_DATA) {
                    writer.println("-- log eof " + nextPage + " type: " + t + " parent: " + p + " expected type: " + Page.TYPE_STREAM_DATA);
                    endOfFile = true;
                    return;
                } else if (k != logKey) {
                    writer.println("-- log eof " + nextPage + " type: " + t + " parent: " + p + " expected key: " + logKey + " got: " + k);
                    endOfFile = true;
                    return;
                }
